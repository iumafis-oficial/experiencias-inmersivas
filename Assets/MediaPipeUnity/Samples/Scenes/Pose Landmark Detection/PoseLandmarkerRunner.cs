...
using System; // <--- Añadido para el Action
...
namespace Mediapipe.Unity.Sample.PoseLandmarkDetection
{
  ...
  {
    // EVENTO PERSONALIZADO: Este es el puente para la toga
    public Action<PoseLandmarkerResult> OnPoseResults;
        ...
        switch (taskApi.runningMode)
        {
          case Tasks.Vision.Core.RunningMode.IMAGE:
            if (taskApi.TryDetect(image, imageProcessingOptions, ref result))
            {
              ...
              OnPoseResults?.Invoke(result); // <--- Disparar evento para modo IMAGE
            }
            else
            {
          ...
          case Tasks.Vision.Core.RunningMode.VIDEO:
            ...
            {
              _poseLandmarkerResultAnnotationController.DrawNow(result);
              OnPoseResults?.Invoke(result); // <--- Disparar evento para modo VIDEO
            }
            else
            {
              _poseLandmarkerResultAnnotationController.DrawNow(default);
    ...
    {
      // --- LÍNEA CLAVE ---
      // Enviamos el resultado a cualquier script suscrito (como la Toga)
      OnPoseResults?.Invoke(result);
